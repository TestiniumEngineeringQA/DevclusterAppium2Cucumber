Feature: OpenApplication

  @BasicTiklamalarIOS01
  Scenario: BasicTiklamalarIOS01
    Given Wait 45 seconds
    Then Element with xpath "//XCUIElementTypeButton[contains(@name, 'Kategoriler')]" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarIOS02
  Scenario: BasicTiklamalarIOS02
    Given Wait 15 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked


  @BasicTiklamalarIOS03
  Scenario: BasicTiklamalarIOS03
    Given Wait 15 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarIOS04
  Scenario: BasicTiklamalarIOS04
    Given Wait 15 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarIOS05
  Scenario: BasicTiklamalarIOS05
    Given Wait 15 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked


  @BasicTiklamalarIOS0Wait
  Scenario: BasicTiklamalarIOSWait
    Given Wait 1020 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroid01
  Scenario: BasicTiklamalarAndroid01
    Given Wait 30 seconds
    Then Element with xpath "//*[contains(@text, 'Güncel Kalın')]" is clicked
    Then Wait 2 seconds


  @BasicTiklamalarAndroid02
  Scenario: BasicTiklamalarAndroid02
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroid03
  Scenario: BasicTiklamalarAndroid03
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroid04
  Scenario: BasicTiklamalarAndroid04+
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroid05
  Scenario: BasicTiklamalarAndroid05
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroid06
  Scenario: BasicTiklamalarAndroid06
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroidFail
  Scenario: BasicTiklamalarAndroidFail
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaignsadsadsad" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroidFail2
  Scenario: BasicTiklamalarAndroidFail2
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaignsadsadsad" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked


  @BasicTiklamalarAndroidFail3
  Scenario: BasicTiklamalarAndroidFail3
    Given Wait 15 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaignsadsadsad" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_campaign" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_trademarks" is clicked
    Then Wait 2 seconds
    Then Element with id "com.gratis.android:id/nav_graph_categories" is clicked

  @BasicTiklamalarIOSFail
  Scenario: BasicTiklamalarIOSFail
    Given Wait 15 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalarsadsadsad']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarIOSFail2
  Scenario: BasicTiklamalarIOSFail2
    Given Wait 15 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalarsadsadsad']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarIOSFail3
  Scenario: BasicTiklamalarIOSFail3
    Given Wait 15 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalarsadsadsad']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Markalar']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kategoriler']" is clicked
    Then Wait 2 seconds
    Then Element with xpath "//XCUIElementTypeButton[@name='Kampanyalar']" is clicked
    Then Wait 2 seconds

  @BasicTiklamalarAndroidWait
  Scenario: BasicTiklamalarAndroidWait
    Given Wait 1020 seconds
    Then Element with xpath "//*[contains(@text, 'Kategoriler')]" is clicked
    Then Wait 2 seconds
